...
public class LocalAnalysisService {
    ...
    public RestaurantAnalysisResponse generateFullAnalysis(List<Map<String, String>> rows,
        ...
        List<ItemPerformance> topRevenueItems = allItems.stream()
                .sorted(Comparator.comparingDouble(ItemPerformance::getRevenue).reversed())
                .limit(5)
                .toList();
        Set<String> topRevenueNames = topRevenueItems.stream().map(ItemPerformance::getItemName).collect(Collectors.toSet());
        ...
        List<ItemPerformance> leastSelling = allItems.stream()
                ...
                .filter(item -> !topRevenueNames.contains(item.getItemName()))
                .limit(3)
                .toList();
        ...
        List<String> profitOpt = insightService.generateProfitOptimizationSuggestions(leastSelling);
